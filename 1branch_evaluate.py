...
model_name = 'PoreFlow_minMax2_branches_1_filters_10_50621_bin_detrendedpot'
dir_data   = 'G:/My Drive/Documents/Research/Solid_Full_X/finney'  #location of the data
features = ['e_pore', 'linear']
...
for ii in range( 0, np.size(samples) ):
    ...
    test_set  = pore_utils.load_data(sets = test_on, features=features, path=dir_data, split=True, 
                                     input_size = input_size, overlap=0 )
    
    
    bnary = train_set['e_pore']
    bnary[bnary <= 0] = 0
    bnary[bnary > 0] = 1
    e_test,  _       = pore_utils.transform(bnary, data_transform_pore, model_name, fileName='bin_stats')
    ...
    ########### Mean velocity calc
    vel_t_pred_mean = vel_t_pred[vel_t_pred != 0].mean()
    vel_t_true_mean = vel_t_true[vel_t_true != 0].mean()
    ...
    rel_error = np.abs((vel_t_pred_full - vel_t_true_full) / vel_t_true_full) * 100
    rel_error_pore = rel_error[vel_t_true_full > 0].mean()
    print(f'Relative Error in the pore space is {rel_error_pore:0.4f} %')
...
#%%
    ...
    slice_true = vel_t_true_full[:,:,45]
    slice_pred = vel_t_pred_full[:,:,45]
    ...
    im=axs[2].imshow((np.abs((slice_true-slice_pred))/np.abs(slice_true)),
                            clim=(0,0.5), cmap=plt.cm.inferno)#,norm=LogNorm(1,100)
    fig.colorbar(im,ax=axs[2])
    axs[2].set_title('Relative error')
    axs[2].axis('off')
    
    print(np.amax(np.abs((slice_true-slice_pred))/np.abs(slice_true)))
